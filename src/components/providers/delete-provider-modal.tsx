...
export function DeleteProviderModal({
  ...
  const appointmentCopy =
    appointmentCount === 1
      ? "1 linked appointment"
      : `${appointmentCount} linked appointments`;
  const deleteDescription =
    appointmentCount > 0
      ? `This will permanently delete ${displayName} and ${appointmentCopy}. This action cannot be undone.`
    ...
    >
      ...
      >
        <Pressable onPress={(event) => event.stopPropagation()} style={styles.modal}>
          ...
          <Text style={styles.description}>{deleteDescription}</Text>
